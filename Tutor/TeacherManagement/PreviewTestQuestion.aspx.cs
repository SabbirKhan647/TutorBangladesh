...
namespace Tutor.TeacherManagement
{
    public partial class PreviewTestQuestion : System.Web.UI.Page
    {
        ...
        {
            DisplayTotalQuesTotalMarks();
            PreviewTestQuestionByTestID();

        }

        private void DisplayTotalQuesTotalMarks()
        {
            SqlConnection c = new SqlConnection(ConfigurationManager.ConnectionStrings["TutorConnectionString"].ConnectionString);
            SqlCommand cmd = new SqlCommand("CalculateTotalQuesTotalMarkForATestSP", c);
            cmd.CommandType = CommandType.StoredProcedure;
            c.Open();
            ...
            cmd.ExecuteNonQuery();
            lbltotalQues1.Text = "Total Questions: " + cmd.Parameters["@totalques"].Value.ToString();
            lblTotalMarks1.Text = "Total Marks: " + cmd.Parameters["@totalWorth"].Value.ToString();
            if (c != null)
            {
                c.Close();
            }
