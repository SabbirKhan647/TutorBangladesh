...
namespace Tutor.TeacherManagement
{
    public partial class CreateNotice : System.Web.UI.Page
    {
        ...
        protected void btnGenerateNotice_Click(object sender, EventArgs e)
        {
            ...
            parm = new SqlParameter("@date", SqlDbType.Date);
            parm.Value = DateTime.Parse  (DateTime .Today.ToString ()).Date ;//get client side date
            parm.Direction = ParameterDirection.Input;
            cmd.Parameters.Add(parm);

            parm = new SqlParameter("@time", SqlDbType.Time);
            parm.Value = DateTime.Parse(DateTime.Now.ToShortTimeString()).TimeOfDay;//get client side time
            parm.Direction = ParameterDirection.Input;
            cmd.Parameters.Add(parm);
